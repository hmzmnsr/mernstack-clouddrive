...
const CreateFolderPopup: React.FC<CreateFolderPopupProps> = ({ onClose }) => {
    ...
    const handleCreateFolder = async () => {
        try {
            ...
            if (error.response) {
                if (error.response.status === 409) { // Assuming 409 is the status code for "Conflict" (folder already exists)
        ...
        <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
            <div className='bg-white h-2/6 w-2/4 py-10 px-20 rounded-lg shadow-lg'>
                <div className='flex flex-col items-start justify-start'>
                    ...
                    {message && (
                        <p className={`mt-3 text-base ${message.includes('successfully') ? 'text-green-600' : 'text-red-600'}`}>
                            {message}
                        </p>
                    )}
                </div>                
